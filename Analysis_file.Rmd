...
1. For the language used in comments and the performance of emotions, positive comments are more concise and emotional, but negative comments being more specific and complex.

  + I found player's comments focus on game experience, story, bugs and update, also in Keyness analysis, positive comments are tend to "love", "great", "fun" while negative comments tend to "bug", "crash", "borning". Also TTR analysis show that the diversity of negative comments in higher than positive ones.

2. Players' comments behaviour is highly event-driven. In version updates, especially large updates can significantly affect the number and the sentiment of comments.

3. There are some comments with a highly similarity according to a cosine analysis test, which means there are some players' comments are from template reply, imitation or community expand. 

4. With STM analysis, I confirmed 4 main topics in all comments which are story and bug (T1), game experience (T2), user review (T3) and atmosphere (T4). T1 takes the biggest proportion, but the heat of disscusion is decrease with version goes by; T2 is increasing which indicates players are transfer their interest from game to themselves. T3 and T4 are stable and represents as the background information of the comments.
